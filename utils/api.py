    ...
    if name in cache.cache_data.keys() and ignore_cache == False:
        ...
        try:
            ...
            if ignore_cache == False:
                cache.cache_data[name] = response.json() # Cache the data for later use
...
def get_pokemon_description(pokemon_name):
    ...
    # Text cleaning
    clean_description = data_utils.clean_text(raw_descriptions)
...
def items_data(name):
    """
    Retrieves and processes data for a given Pokémon item.
